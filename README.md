# Join job results

[![Action Template](https://img.shields.io/badge/Action%20Template-Python%20Container%20Action-blue.svg?colorA=24292e&colorB=0366d6&style=flat&longCache=true&logo=data:image/png;base64,)](https://github.com/jacobtomlinson/python-container-action)
[![CI](https://github.com/luisrayas3/join-job-results/actions/workflows/ci.yaml/badge.svg)](https://github.com/luisrayas3/join-job-results/actions)

This reusable action provides a basic but useful functionality
which is to determin if the results of multiple jobs were all success.

In this way, you can setup your required checks (e.g. for PR merge)
to require a single status check (the one for this job)
and modify the exact checks that are needed
simply by modifying the jobs that go into `needs`.

## Example workflow

```yaml
name: Foo workflow
on: [push, pull_request]
jobs:
  test-1:
    runs-on: ubuntu-latest
    steps:
      - run: execute test1
  test-2:
    runs-on: ubuntu-latest
    steps:
      - run: execute test2
  required-checks:
    needs: [test-1, test-2]
    if: always()
    runs-on: ubuntu-latest
    steps:
    - name: All required checks succeeded
      uses: luisrayas3/join-job-results@master
      with:
        results: ${{ toJson(needs.*.result) }}
```
